...
document.addEventListener("click", function (event) {
  const leadLink = event.target.closest('a[href^="mailto:"]');
  ...
  <header class="site-header">
    <div class="container nav-wrap">
      <a class="logo" href="index.html" aria-label="SAMIVRAH home">
        <img src="images/samivrah-logo.png"
      ...
      <a class="button button-dark header-cta"
  ...
  <footer class="site-footer">
    <div class="container footer-grid">
      <div class="footer-col">
        ...
        <p>
          Recruitment, web solutions and digital growth support
          for service businesses across India.
        </p>
      </div>
    ...
    <div class="container footer-bottom">
      <span>
        © <span id="current-year"></span> SAMIVRAH.
        All rights reserved.
      </span>
      <span>
        Business solutions built around real requirements.
      </span>
    </div>
  </footer>
...
document.querySelectorAll(".lead-form").forEach((form) => {
  // Convert old phone fields into email fields
  const oldPhoneInput =
    form.querySelector('input[name="phone"]');

  if (oldPhoneInput) {
    oldPhoneInput.name = "email";
    oldPhoneInput.type = "email";
    oldPhoneInput.inputMode = "email";
    oldPhoneInput.placeholder = "Email address";
    oldPhoneInput.autocomplete = "email";

    const oldPhoneLabel =
      oldPhoneInput.closest("label");

    if (oldPhoneLabel && oldPhoneLabel.firstChild) {
      oldPhoneLabel.firstChild.textContent =
        "Email address *";
    }
  }
  ...
  form.addEventListener("submit", (event) => {
    ...
    const message = [
      ...
      `Service: ${service}`,
      `Requirement: ${
        requirement || "Please contact me to discuss"
      }`
    ].join("\n");
...
const popup =
  document.getElementById("recruitment-popup");
...
if (popup) {
  const seen =
    sessionStorage.getItem("samivrah-popup-seen");
